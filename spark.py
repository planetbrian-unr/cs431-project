...
def load_table_into_spark(db_name: str, table_name:str):
    # locate present session from initialization function
    spark = SparkSession.builder.getOrCreate()

    df = spark.read.format("jdbc").option("url", db_name).option("dbtable", table_name).option("driver", "org.sqlite.JDBC").load()
    return df
